...
### 🎨 视频水印（动态实时）
- **预览叠加**：实时在预览界面显示水印信息
- **动态水印烧录**：每秒记录GPS和时间数据，使用FFmpeg+ASS字幕将动态水印永久写入视频流
- **真实动态内容**：水印中的时间戳、GPS坐标、速度等信息随录制过程实时变化
- **字幕导出**：可选生成SRT字幕文件，包含所有水印数据
- 显示内容：时间戳（精确到秒）、GPS坐标、速度、方向、摄像头位置
- 支持自定义文本（如车牌号、车辆信息）
- 四种位置选择（左上/右上/左下/右下）
- 两种水印方式：动态烧录到视频（推荐，满足合规要求）或字幕文件
...
└── utils/              # 工具类
    ...
    ├── VideoWatermarkEncoder.kt         # 视频水印编码器（备用）
    ├── WatermarkVideoRecorder.kt        # 带水印的视频录制器（备用）
    ├── FFmpegWatermarkBurner.kt         # FFmpeg静态水印烧录器（已弃用）
    ├── DynamicWatermarkGenerator.kt     # 动态水印数据生成器（ASS格式）
    ├── DynamicWatermarkBurner.kt        # 动态水印烧录器（FFmpeg+ASS）
    └── WatermarkSubtitleGenerator.kt    # SRT字幕文件生成器
```
...
使用FFmpegKit实现动态水印烧录：
- 录制过程中每秒记录GPS和时间数据
- 生成ASS格式字幕文件，精确记录每秒的水印内容
- 使用FFmpeg的ass滤镜将字幕永久烧录到视频中
- 水印内容随时间动态变化（时间戳、GPS坐标、速度等）
- H.264编码保持视频兼容性
- 满足法规对行车记录仪水印的动态实时要求
...
#### 水印设置
- **启用水印**：是否在视频上显示水印
- **显示时间戳**：在水印中显示日期和时间（每秒动态更新）
- **显示GPS坐标**：在水印中显示位置信息（每秒根据GPS实时更新）
- **显示速度**：在水印中显示当前速度（每秒更新）
- **显示摄像头名称**：标识视频来源（前/后/左/右）
- **自定义文本**：添加自定义文本（如车牌号）
- **水印位置**：选择水印显示位置（左上/右上/左下/右下）
- **烧录水印到视频**：使用FFmpeg将动态水印永久写入视频文件
  - 满足法规要求，水印随时间动态变化
  - 录制时每秒采集数据，生成ASS字幕
  - 段落结束后自动烧录（处理时间约为视频时长的20-30%）
...
1. **动态烧录到视频**（推荐，满足法规要求）：
   - **实时数据采集**：录制过程中每秒自动记录当前的GPS坐标、速度、时间戳等信息
   - **ASS字幕生成**：将每秒的水印数据保存为ASS格式字幕文件，精确到秒级
   - **FFmpeg烧录**：使用FFmpeg的ass滤镜将字幕永久烧录到视频流中
   - **真实动态效果**：视频播放时，水印内容每秒都会更新，显示当时的实际GPS位置和时间
   - **永久嵌入**：水印成为视频的一部分，无法移除或篡改
   - **法规合规**：满足法律法规对行车记录仪动态水印的要求
   - **自动处理**：每段视频录制完成后自动处理，无需手动操作
...
2. **SRT字幕文件**（可选，仅用于兼容性）：
   ...
   - 适合需要灵活显示水印的场景
   - 注意：SRT字幕可以被删除，不满足法规的强制要求

**技术细节**：
- 数据采集频率：每秒1次
- 时间精度：精确到秒
- GPS数据：纬度、经度、速度、方向
- 字幕格式：ASS（Advanced SubStation Alpha），支持丰富样式
- 视频编码：H.264，preset=medium，CRF=23
- 音频处理：直接复制，不重新编码，保证无损
